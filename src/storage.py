...
class SQLiteStorage(StoragePort):
    ...
    def save_file(self, file_path: str, cmm_entity: CMMEntity) -> None:
        ...
        try:
            ...
            for entity in cmm_entity.entities:
                self._save_entity_recursive(cursor, entity, file_path, now, parent_id=None)
    ...
    def _save_entity_recursive(self, cursor: sqlite3.Cursor, entity: Dict[str, Any], file_path: str, now: str, parent_id: Optional[str]):
        ...
        cursor.execute("""
            INSERT INTO entities_v3 (id, name, type, visibility, parent_id)
            VALUES (?, ?, ?, ?, ?)
        """, (entity_id, name, entity_type, visibility, parent_id))
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (entity_id, file_path, raw_docstring, signature, cmm_type, method_kind, now, now))
        ...
        for method in methods:
            self._save_entity_recursive(cursor, method, file_path, now, parent_id=entity_id)
    ...
    def upsert_file(self, file_path: str, cmm_entity: CMMEntity) -> None:
        ...
        # Check existing file
        cursor.execute("SELECT id, file_hash FROM files_v3 WHERE file_path = ?", (file_path,))
        row = cursor.fetchone()
        ...
        if row:
            ...
            cursor.execute("SELECT entity_id FROM metadata WHERE file_path = ?", (file_path,))
            ...
            if entity_ids:
                ...
                cursor.execute(f"DELETE FROM entities_v3 WHERE id IN ({placeholders})", entity_ids)
    ...
    def get_file(self, file_path: str) -> Optional[CMMEntity]:
        ...
        cursor.execute("SELECT schema_version FROM files_v3 WHERE file_path = ?", (file_path,))
        row = cursor.fetchone()
        if not row:
            conn.close()
            return None
        ...
        if entities_map:
            ...
            c_rel = conn.execute(f"SELECT from_id, to_name, rel_type FROM relations WHERE from_id IN ({placeholders})", list(entities_map.keys()))
