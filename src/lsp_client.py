...
class Location:
    ...
    def from_lsp_response(cls, result: Dict[str, Any]) -> Optional["Location"]:
        ...
        return cls(
            uri=uri, line=start.get("line", 0), character=start.get("character", 0)
        )
...
class LSPClient:
    ...
    def open_document(self, file_uri: str, content: str):
        """
        Send textDocument/didOpen notification to LSP server.

        Pyright requires files to be "opened" before querying definitions.
