...
def fix_database(db_path: str):
    ...
    try:
        ...
        if "line_start" not in columns:
            ...
            cursor.execute("ALTER TABLE entities_v3 ADD COLUMN line_start INTEGER DEFAULT 0")
        
        if "line_end" not in columns:
            print("Adding line_end column...")
            cursor.execute("ALTER TABLE entities_v3 ADD COLUMN line_end INTEGER DEFAULT 0")

        # 3. Create Unique Index
        print("Creating unique index on relations...")
        cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_relations_unique ON relations(from_id, to_name, rel_type);")
